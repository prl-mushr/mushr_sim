...
class SimCarPose:
    def __init__(self):
        ...
        self.STEERING_ANGLE_OFFSET = float(
            rospy.get_param("~steering_angle_offset", 0.00001)
        )
        self.STEERING_ANGLE_NOISE = float(
            rospy.get_param("~steering_angle_noise", 0.00001)
        )
        ...
        self.cur_pose_pub = rospy.Publisher(
            "/sim_car_pose/pose", PoseStamped, queue_size=1
        )
    ...
    def init_pose_cb(self, msg):
        ...
        if (
            ...
            if (
                ...
                and (self.last_steering_angle is not None)
            ):
                self.can_step = True
    ...
    def timer_cb(self, event):
        ...
        if np.abs(delta) < 1e-2:
            ...
            dx = (self.CAR_LENGTH / tandelta) * (
                np.sin(self.cur_pose[2] + dtheta) - np.sin(self.cur_pose[2])
            )
            dy = (self.CAR_LENGTH / tandelta) * (
                -1 * np.cos(self.cur_pose[2] + dtheta) + np.cos(self.cur_pose[2])
            )
